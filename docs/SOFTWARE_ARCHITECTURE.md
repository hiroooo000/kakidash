...
- **ViewportService**: Handles viewport control such as zoom, pan, and animation loop.
- **NavigationService**: Handles navigation logic between nodes based on direction.
- **ImageStore**: Component that manages original high-resolution image data in memory. Separates binary data from the model to prevent JSON bloat.
- **ImageProcessingService**: Handles client-side image resizing and thumbnail generation.
...
### 4.7 Image Paste and Persistence Flow (Hybrid Storage)

Flow for handling image pastes by separating thumbnails (inside JSON) and original data (external).
...
sequenceDiagram
    ...
    Note over Store: Keep original in memory
    
    Controller->>Service: addImageNode(thumbnail, imageRef, size)
    Note over Service: JSON stores only ref ID and thumbnail
    
    Controller->>Controller: render()
    
    Note over User, Host: During Save
    ...
    Note over Host: Write separately to sidecar folder
```

### 4.8 Image Garbage Collection Flow

Flow for removing unused images from memory and the file system.
...
sequenceDiagram
    ...
    Note over Store: Remove from memory
    
    Note over Controller, Host: During Save
    Host->>Controller: getImages()
    Controller-->>Host: Map of currently used images
    Note over Host: Sync sidecar folder (delete removed images)
```
