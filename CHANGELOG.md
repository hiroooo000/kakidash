...
- Fix image zoom interaction bug introduced in v0.4.0 where pressing the space key on image nodes failed to trigger the zoom modal.

## [0.4.0] - 2026-04-26

### Added

- **Hybrid Image Storage**: Implemented a new strategy to separate large image binaries from the main JSON model to improve performance and git-friendliness.
- **New Public APIs**:
  - `getImages()`: Retrieves all original high-resolution images currently in memory.
  - `gcImages()`: Cleans up unused images from the internal store.
  - `zoomNode(nodeId)`: Explicitly trigger the image zoom modal for a specific node.
- **New Constructor Options**:
  - `getImage`: A callback option to fetch original images from the host environment (e.g., VSCode sidecar folders).
- **Legacy Migration**: Automatic migration of old `.kaki` files (with embedded large images) to the new hybrid format during `loadData()`.

### Changed

- **Internal Data Schema**: Nodes now store `thumbnail` (small Base64) and `imageRef` (ID) instead of the full `image` data.

### Fixed

- Fix layout calculation where image nodes were rendered with incorrect heights.
- Fix interaction on image nodes: double-clicking or pressing Space now correctly triggers the zoom modal instead of opening the text editor.
- Fix various typing errors in test files related to new API interfaces.
...
- Fix markdown export dialog to explicitly allow selecting `.md` files.

## [0.3.1] - 2026-03-16

### Added

- Introduce drag threshold to prevent accidental dragging during clicks and double-clicks.

### Fixed

- Fix clipboard paste issues where image metadata could be mixed or external text could overwrite internal data.
